...
class TestWebExtractSecretExfil:
    ...
    @pytest.mark.asyncio
    async def test_blocks_credential_named_query_param_before_third_party_dispatch(self):
        """Signed URLs stay local; extract providers must not receive opaque credentials."""
        from tools.web_tools import web_extract_tool

        with patch("tools.web_tools_extract._resolve_extract_provider") as resolve:
        ...
        assert "opaque-oauth-value" not in result
