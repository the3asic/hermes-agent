...
def _configure_resolution(
    ...
    *,
    ...
    monkeypatch.setattr(bt_install, "_merge_browser_path", merge_path)
...
def test_runnable_033_remains_generic_but_is_rejected_for_pin(monkeypatch, tmp_path):
    ...
    assert bt_install._find_agent_browser() == old_cli
    with pytest.raises(bt_install.AgentBrowserCapabilityError) as exc_info:
        bt_install._find_agent_browser(require_pin_tab=True)
    ...
    real_probe = bt_install._probe_agent_browser_version
    monkeypatch.setattr(
        bt_install,
        "_probe_agent_browser_version",
        lambda _path: pytest.fail("negative cache should suppress a second probe"),
    )
    with pytest.raises(bt_install.AgentBrowserCapabilityError, match="agent-browser >=0.34.0"):
        bt_install._find_agent_browser(require_pin_tab=True)
    clock[0] += bt.AGENT_BROWSER_PIN_TAB_FAILURE_TTL_SECONDS + 0.01
    monkeypatch.setattr(bt_install, "_probe_agent_browser_version", real_probe)
    assert bt_install._find_agent_browser(require_pin_tab=True) == old_cli


def test_generic_install_guidance_keeps_node22_package_line(monkeypatch):
    monkeypatch.setattr(bt_install, "_is_termux_environment", lambda: False)

    assert bt_install._browser_install_hint() == (
...
def test_pin_lookup_falls_through_033_to_034_and_caches(monkeypatch, tmp_path):
    ...
    assert bt_install._find_agent_browser() == old_cli
    assert bt_install._find_agent_browser(require_pin_tab=True) == new_cli
...
def test_agent_browser_034_on_node22_is_not_pin_capable(monkeypatch, tmp_path):
    ...
    with pytest.raises(bt_install.AgentBrowserCapabilityError, match=r"Detected Node\.js 22"):
        bt_install._find_agent_browser(require_pin_tab=True)
...
def test_npx_pin_capability_respects_node_floor(
    ...
    if pin_available:
        ...
        with pytest.raises(
            bt_install.AgentBrowserCapabilityError, match=r"Detected Node\.js 22"
        ):
            bt_install._find_agent_browser(require_pin_tab=True)
        ...
        assert bt_install._find_agent_browser() == bt.NPX_AGENT_BROWSER_SENTINEL
