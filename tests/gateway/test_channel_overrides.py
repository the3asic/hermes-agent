...
class TestGetChannelOverride:
    ...
    def test_telegram_topic_beats_parent_group_and_inherits_missing_fields(self):
        config = GatewayConfig(
            platforms={
                Platform.TELEGRAM: PlatformConfig(
                    enabled=True,
                    channel_overrides={
                        "group_1": ChannelOverride(
                            model="parent-model",
                            system_prompt="Parent prompt",
                        ),
                        "topic_9": ChannelOverride(model="topic-model"),
                    },
                ),
            },
        )
...
class TestGetSystemPromptForChannel:
    ...
    def test_exact_entry_can_inherit_parent_prompt(self):
        config = GatewayConfig(
            platforms={
                Platform.DISCORD: PlatformConfig(
                    enabled=True,
                    channel_overrides={
                        "parent": ChannelOverride(system_prompt="Parent prompt"),
    ...
    def test_empty_exact_prompt_suppresses_parent_and_global_prompt(self):
        config = GatewayConfig(
            platforms={
                Platform.DISCORD: PlatformConfig(
                    enabled=True,
                    channel_overrides={
                        "parent": ChannelOverride(system_prompt="Parent prompt"),
                        "thread": ChannelOverride(system_prompt=""),
                    },
                ),
            },
        )
...
class TestResolveSessionAgentRuntimePriority:
    ...
    def test_exact_provider_inherits_parent_model(self):
        runner = object.__new__(GatewayRunner)
        runner._session_model_overrides = {}
        runner.config = GatewayConfig(
            platforms={
                Platform.DISCORD: PlatformConfig(
                    ...
                    channel_overrides={
                        ...
                        "thread": ChannelOverride(provider="openrouter"),
                    },
                ),
            },
        )
...
class TestLoadedDiscordChannelContract:
    def test_yaml_load_resolves_all_precedence_and_real_agent_chain(
        ...
        assert (
            runner._resolve_session_fallback_chain(
        ...
        stored = (
            ...
            .channel_overrides["exact-thread"]
            .fallback_providers
        )
...
class TestDiscordReasoningOverrides:
    ...
    def test_omitted_channel_uses_global_reasoning(self, monkeypatch):
        config = GatewayConfig(
            platforms={Platform.DISCORD: PlatformConfig(enabled=True)},
        )
        runner = object.__new__(GatewayRunner)
    ...
    def test_non_discord_channel_reasoning_uses_same_contract(self):
...
class TestChannelFallbackOverrides:
    @staticmethod
    def _runner(config):
        runner = object.__new__(GatewayRunner)
        runner.config = config
    ...
    def test_explicit_empty_disables_parent_and_global(self):
        ...
        assert (
            self._runner(config)._resolve_session_fallback_chain(
                source=self._source("thread", parent_id="parent"),
                global_chain=[{"provider": "global", "model": "global-model"}],
            )
            == []
    ...
    def test_non_discord_channel_fallback_uses_same_contract(self):
