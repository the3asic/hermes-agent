...
Every view must keep its own user-visible rejection strings and disable/grey
expired buttons. Clarify keeps accepting a text answer after button expiry.
"""
...
async def test_on_timeout_disables_and_greys_embed(name):
    ...
    expected_footer = (
        "⏱ Buttons expired — reply with your answer"
        if name == "clarify" else "⏱ Prompt expired — no action taken"
    )
    assert embed.footer == expected_footer
