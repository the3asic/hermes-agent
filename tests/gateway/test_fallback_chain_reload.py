...
def test_main_turn_reuses_cached_agent_and_reapplies_resolved_channel_chain(
    tmp_path, monkeypatch
):
    """A cache hit reapplies the runner's resolved in-memory channel policy.

    Channel override YAML is startup-loaded; this test intentionally does not
    claim file hot-reload behavior.
    """
    ...
    class CapturingCachedAgent:
        ...
        def run_conversation(
            ...
            self.turn_chains.append([dict(entry) for entry in self._fallback_chain])
    ...
    platform_config = PlatformConfig(
        ...
        channel_overrides={
            "channel": ChannelOverride(
                ...
                fallback_providers=first_chain,
            ),
        },
    )
    ...
    assert (
        CapturingCachedAgent.instances[0].init_kwargs["ephemeral_system_prompt"]
        == "Channel prompt"
    )
    assert CapturingCachedAgent.instances[0].turn_chains == [first_chain, second_chain]

    # A real AIAgent sets this flag when it activates a fallback. A successful
    # fallback turn must evict the cached agent so the next turn retries the
    # resolved primary instead of remaining pinned to the fallback runtime.
    CapturingCachedAgent.instances[0]._fallback_activated = True
    third_result = asyncio.run(runner._run_agent(**run_kwargs))
    assert third_result["final_response"] == "ok"
    assert len(CapturingCachedAgent.instances) == 1

    fourth_result = asyncio.run(runner._run_agent(**run_kwargs))
    assert fourth_result["final_response"] == "ok"
    assert len(CapturingCachedAgent.instances) == 2
    assert CapturingCachedAgent.instances[1]._fallback_activated is False
