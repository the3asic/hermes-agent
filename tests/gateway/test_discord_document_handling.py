...
class TestIncomingDocumentHandling:
    ...
    @pytest.mark.asyncio
    async def test_platform_extra_attachment_values_override_env_and_top_level_yaml(
        self, adapter, tmp_path, monkeypatch
    ):
        """Platform extras retain their existing priority over legacy inputs."""
        ...
        assert "[Content of notes.txt]:" not in event.text
        assert "visible text" not in event.text
