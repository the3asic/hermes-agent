...
class TestRuntimeFtsRebuild:
    ...
    def test_append_defers_rebuild_after_fts_corruption(
        ...
        # The upstream housekeeping retry must honor the same offline-only policy.
        monkeypatch.setattr(db, "_recover_stale_fts_locked", lambda *a, **k: pytest.fail("live tick must not rebuild"))
