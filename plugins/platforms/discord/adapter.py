...
from plugins.platforms.discord.adapter_context import DiscordContextMixin


class DiscordAdapter(DiscordContextMixin, DiscordMediaMixin, BasePlatformAdapter):
...
def _define_discord_view_classes() -> None:
    ...
    class ClarifyChoiceView(_HermesView):
        ...
        async def on_timeout(self):
            self.resolved = True
            self._disable_all()
            # Button expiry must not strand the still-pending gateway question.
            try:
                from tools.clarify_gateway import mark_awaiting_text
                mark_awaiting_text(self.clarify_id)
            except Exception:
                logger.debug("Clarify text fallback failed for %s", self.clarify_id, exc_info=True)
            await self._expire_embed("⏱ Buttons expired — reply with your answer")
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    for config_key, env_key in (
        ...
        ("history_backfill_free_response", "DISCORD_HISTORY_BACKFILL_FREE_RESPONSE"),
        ("history_backfill_limit", "DISCORD_HISTORY_BACKFILL_LIMIT"),
        ("inline_text_attachments", "DISCORD_INLINE_TEXT_ATTACHMENTS"),
        ("max_attachment_bytes", "DISCORD_MAX_ATTACHMENT_BYTES"),
    ):
        value = discord_cfg.get(config_key, platform_extra_cfg.get(config_key))
        if value is not None:
            seeded_extra[config_key] = value
            if not _skip_env_bridge:
