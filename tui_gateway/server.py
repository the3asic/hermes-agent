...
def _resolve_runtime_with_fallback(resolve_kwargs: dict | None = None) -> _RuntimeFallbackResolution:
    ...
    try:
        return _RuntimeFallbackResolution(resolve_runtime_provider(**(resolve_kwargs or {})), None, False, None)
...
def _resolve_agent_model_runtime(model_override, provider_override) -> tuple[str, dict, dict | None]:
    """(model, runtime, fallback_entry) for a new agent; a per-session override (/model switch or a resumed row's persisted
    ...
    if resolution.used_fallback:
        ...
        return resolution.selected_model, resolution.runtime, resolution.fallback_entry
    resolution.runtime.update({k: v for k, v in overrides.items() if v})
    return model, resolution.runtime, None
...
def _make_agent(
    ...
    model, runtime, fallback_entry = _resolve_agent_model_runtime(model_override, provider_override)
    if fallback_entry is not None:
        from hermes_constants import resolve_fallback_reasoning_config
        effective_reasoning = resolve_fallback_reasoning_config(cfg, str(model or ""), fallback_entry)
    else:
        effective_reasoning = (
            reasoning_config_override if reasoning_config_override is not None
    ...
    apply_initial_reasoning_policy_provenance(agent, fallback_entry, effective_reasoning)
    return agent
