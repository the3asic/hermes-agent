...
This is metadata about a top-N search response, not proof that a snippet is current or correct and not evidence that Hermes fetched the linked page. Bare provider claims named `confidence`, `fresh`, `current`, `verified`, or `authoritative` are removed throughout the successful provider payload. An upstream cache timestamp is exposed only when it is a supported timezone-qualified RFC 3339 date-time. Malformed values become `null` with an explicit invalid status. Hermes does not validate leap-second occurrence tables, so any `time-second=60` notation is also `null` with the neutral status `reported_unsupported_second_60`; that status does not claim the represented instant was a valid leap second.
