...
def _completion_status(evt: dict) -> str:
    ...
    return "is no longer running" if evt.get("exit_code", "?") is None else "exited"
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _attribution and isinstance(_out, str) and len(_out) > 600:
        _out = (
            ...
            + _out[-600:])
    exit_detail = "exit status unavailable" if _exit is None else f"exit code {_exit}{_signal}"
    return (
        f"[IMPORTANT: Background process {_sid} {_completion_status(evt)} ({exit_detail}).\n"
        f"{attribution}Command: {_cmd}\nOutput:\n{_out}]")
