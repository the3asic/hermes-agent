...
class _BrowserSessionBackend:
    ...
    def ensure_healthy(self) -> bool:
        ...
        if not _lifecycle._cleanup_browser_session_keys(
            ...
            raise _lifecycle._BrowserSessionRetiredError(self._session_key)
        return False
...
_cleanup_lock = threading.RLock()  # session state and nested lifecycle transitions
...
_browser_task_states: Dict[str, _lifecycle.BrowserTaskState] = {}


_browser_task_generations: Dict[str, int] = {}


_browser_task_cleanup_reasons: Dict[str, _lifecycle.BrowserCleanupReason] = {}
...
_pending_provider_cleanups: Dict[
    tuple[str, int, str], _lifecycle._PendingProviderCleanup
] = {}
...
def _attach_auto_snapshot(response: Dict[str, Any], nav_session_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if snap_result.get("code") in {"tab_gone", "browser_session_retired"}:
            return _lp._copy_fallback_warning(
                {**response, **_err(snap_result.get("error", "Pinned browser tab disappeared after navigation"))},
                snap_result,
            )
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        restarting_retired_task = _lifecycle._clear_retired_browser_task_for_navigation(effective_task_id)
    ...
    try:
        ...
        if restarting_retired_task:
            _lifecycle._restore_retired_browser_task_after_failed_navigation(effective_task_id, nav_session_key)
        raise
    ...
    if _lifecycle._is_task_owned_shared_cdp_session(session_info):
        target_id = _cdp._pinned_cdp_target_id(nav_session_key)
        if target_id:
            _cdp._ensure_cdp_supervisor(
                nav_session_key, target_id=target_id,
    ...
    if snapshot_failure is not None:
        if restarting_retired_task:
            _lifecycle._restore_retired_browser_task_after_failed_navigation(effective_task_id, nav_session_key)
        return _dumps(snapshot_failure)
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    for result in (console_result, errors_result):
        if result.get("code") in {"tab_gone", "browser_session_retired"}:
            return _failed_response(result, "Pinned browser tab is gone")
...
def _eval_supervisor_fast_path(effective_task_id: str, expression: str) -> Optional[str]:
    """Evaluate once; fall back only on structured evidence of no dispatch."""
    try:
    ...
    with _lifecycle._task_cleanup_operation_lock(bare_task_id):
        ...
        try:
            ...
            return _dumps({
                **_err(f"{type(exc).__name__}: {exc}"), "code": "cdp_evaluate_failed",
                "data": {"exception_type": type(exc).__name__},
            })
    if result.get("ok"):
        return _eval_result_or_blocked(
            effective_task_id, _parse_eval_value(result.get("result")), {}, method="cdp_supervisor")
    error = result.get("error") or "evaluate_runtime failed"
    kind, data = result.get("kind"), result.get("data")
    if kind == "js_exception":
        return _dumps({**_err(error), "code": "javascript_exception", "data": data or {}})
    if kind == "cdp_protocol" and isinstance(data, dict) and data.get("session_lost") is True:
        _cdp._stop_cdp_supervisor(effective_task_id)
    elif kind != "supervisor_unavailable":
        return _dumps({**_err(error), "code": kind or "cdp_evaluate_failed", "data": data or {}})
    logger.debug("browser_eval: supervisor path unavailable (%s), falling back to subprocess", error)
    return None
...
def _eval_failure_response(result: Dict[str, Any]) -> str:
    ...
    if any(hint in err.lower() for hint in ("unknown command", "not supported", "not found", "no such command")):
        ...
        # Serialization can fail after side effects. Neither path may replay the expression.
        err = (
