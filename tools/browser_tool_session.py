...
def _agent_browser_argv(browser_cmd: str, *, pin_tab: bool = False) -> list:
    ...
    absent the bare name gives a readable ``FileNotFoundError``. Reviewed exact packages
    still run with install-time lifecycle scripts disabled.
    """
    if _install._is_npx_agent_browser_sentinel(browser_cmd):
        _npx_bin = _install._resolve_npx_bin() or "npx"
        spec = _bt.AGENT_BROWSER_PIN_TAB_NPX_SPEC if pin_tab else _bt.AGENT_BROWSER_NPX_SPEC
...
def _agent_browser_command_env(socket_dir: str, *, pin_tab: bool = False) -> Dict[str, str]:
    ...
    _install._apply_agent_browser_npm_policy(env, pin_tab_acquisition=pin_tab)
...
def _create_cloud_session_or_fallback(task_id: str, provider) -> Dict[str, Any]:
    ...
    try:
        ...
        _lifecycle._dispose_unpublished_session(task_id, session_info, provider)
...
def _get_session_info(
    ...
    *,
    ...
    with _bt._cleanup_lock:
        ...
        if state in {_lifecycle.BrowserTaskState.RETIRING, _lifecycle.BrowserTaskState.RETIRED} and not cleanup_lookup:
    ...
    if existing_session is not None and not _for_cleanup and not _bt._browser_session_backend(task_id).ensure_healthy():
    ...
    with _bt._cleanup_lock:
        ...
        state = _lifecycle._task_state_locked(bare_task_id)
        if state in {_lifecycle.BrowserTaskState.RETIRING, _lifecycle.BrowserTaskState.RETIRED}:
            raise _lifecycle._BrowserSessionRetiredError(task_id)
        ...
        if not _lifecycle._task_has_active_sessions_locked(bare_task_id):
            _lifecycle._set_task_state_locked(bare_task_id, _lifecycle.BrowserTaskState.STARTING)
    ...
    # Slow provider creation is outside the lock; publication is generation-fenced.
    try:
        session_info = _create_session_for_key(task_id, force_local)
    except BaseException:
        _lifecycle._abandon_session_creation(bare_task_id, creation_generation)
        raise
    ...
    with _bt._cleanup_lock:
        ...
        if not stale_creator and winner is None:
            ...
            # A newly published session is healthy by definition. Clear any
            # suspect marker left by a timed-out predecessor under this key.
            _bt._suspect_browser_sessions.pop(task_id, None)
            _lifecycle._set_task_state_locked(bare_task_id, _lifecycle.BrowserTaskState.ACTIVE)
            published = True
        else:
            published = False
...
def _discard_timed_out_browser_session(task_id: str, session_info: Dict[str, Any], task_socket_dir: str) -> None:
    ...
    with _bt._cleanup_lock:
        ...
        if _lifecycle._is_task_owned_shared_cdp_session(session_info):
            # The named daemon and target file are the exact-close recovery path.
            # Rotating/deleting them would orphan a page in an external browser.
            _bt.logger.warning("Shared-CDP command timed out for %s; retaining pinned ownership for retry/cleanup", task_id)
            return
...
def _browser_command_preflight(*, allow_cleanup: bool = False) -> Dict[str, Any]:
    ...
    if not allow_cleanup and is_interrupted():
...
def _spawn_and_collect(
    ...
    browser_env = _agent_browser_command_env(
        task_socket_dir, pin_tab=_lifecycle._is_task_owned_shared_cdp_session(session_info),
    )
...
def _run_browser_command(
    ...
    preflight = _browser_command_preflight(allow_cleanup=_allow_cleanup)
    if "browser_cmd" not in preflight:
        return preflight
    browser_cmd = preflight["browser_cmd"]

    try:
        session_info = _get_session_info(task_id, _for_cleanup=True) if _allow_cleanup else _get_session_info(task_id)
    ...
    if not _allow_cleanup and command != "close" and session_info.get("cdp_url"):
        _cdp._ensure_cdp_supervisor(task_id)

    if not _allow_cleanup and _lifecycle._is_browser_task_unavailable(task_id):
        return _lifecycle._browser_session_retired_result(task_id)
    pinned = _lifecycle._is_task_owned_shared_cdp_session(session_info)
    if pinned:
        try:
            browser_cmd = _install._find_agent_browser(require_pin_tab=True)
        except _install.AgentBrowserCapabilityError as exc:
            return {
                "success": False, "error": str(exc), "code": "pin_tab_unavailable",
                "data": {"required_agent_browser": ">=0.34.0", "required_node": ">=24", "non_cdp_available": True},
            }

    # Shared CDP owns one named pinned target. Cloud providers own an entire browser
    # and retain their older generic CDP contract without requiring Node 24.
    ...
    cmd_parts = _agent_browser_argv(browser_cmd, pin_tab=pinned) + backend_args + ["--json", command] + args
