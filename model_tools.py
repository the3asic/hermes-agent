...
def _execute_tool(function_name: str, function_args: Dict[str, Any], original_args: Dict[str, Any], ids: _CallIds,
    ...
    def _dispatch(next_args: Dict[str, Any]) -> Any:
        ...
        dispatched = registry.dispatch(function_name, next_args, **dispatch_kwargs)
        if function_name == "web_search":
            raw_dispatch_results.append(dispatched)
        return dispatched
    ...
    with _approval_observability(ids):
        ...
        result = run_tool_execution_middleware(function_name, function_args, _dispatch, original_args=original_args,
                                               **ids.hook_kwargs())
        if function_name == "web_search":
            if not raw_dispatch_results:
                if _is_successful_web_search_result(result):
                    logger.warning(
                        "tool_execution middleware short-circuited web_search with a successful result; "
                        "the wrapper truth contract did not generate the model-bound output"
                    )
            elif _web_search_transform_changes_success_result(raw_dispatch_results[-1], result):
                logger.warning(
                    "tool_execution middleware crossed the successful web_search wrapper truth boundary; "
                    "wrapper provenance may not describe the model-bound output"
                )
        return result
...
def _apply_transform_tool_result_hook(function_name: str, function_args: Dict[str, Any], result: Any, duration_ms: int,
    ...
    try:
        ...
        if has_hook("transform_tool_result"):
            ...
            if function_name == "web_search" and _web_search_transform_changes_success_result(result, transformed):
                logger.warning(
                    "transform_tool_result crossed the successful web_search wrapper truth boundary; "
                    "wrapper provenance may not describe the model-bound output"
                )
            return transformed
    except Exception as _hook_err:
