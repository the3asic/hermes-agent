...
class LSPService:
    ...
    async def _open_and_wait_async(
        ...
        """Open a file and return only diagnostics for its current version.

        ``None`` means the server produced no fresh verdict before the wait
        budget expired; ``[]`` means it checked the file and found no errors.
        """
        async with self._leased_client(file_path) as client:
            if client is None:
                return None
