    ...
    if resp_usage:
        ...
        for source_field, target_field in (
            ...
            if isinstance(resp_usage, dict):
                ...
                if (fields_set is None or source_field in fields_set) and hasattr(resp_usage, source_field):
...
def _fallback_request_kwargs(
    ...
    fb_label: str, fallback_route_ref: Optional[_FallbackRouteRef], apply_fast_lane: bool,
) -> Dict[str, Any]:
    """Build request kwargs for one fallback destination (cache-section replan + fast-lane cap)."""
    fallback_max_tokens, fallback_extra_body, fallback_reasoning = _fallback_request_controls(
        task, destination, fb_label, max_tokens=max_tokens,
        effective_extra_body=effective_extra_body, reasoning_config=reasoning_config,
        fallback_route_ref=fallback_route_ref, apply_fast_lane=apply_fast_lane,
    )
    ...
    fb_kwargs = _build_call_kwargs(
        ...
        extra_body=fallback_extra_body, reasoning_config=fallback_reasoning, base_url=destination.base_url, task=task)
...
def _plan_fallback_candidate(
    ...
    effective_timeout: float, apply_fast_lane: bool,
    fallback_route_ref: Optional[_FallbackRouteRef] = None, **request,
    ...
    common = dict(
        task=task, effective_timeout=effective_timeout, fb_label=fb_label,
        fallback_route_ref=fallback_route_ref, apply_fast_lane=apply_fast_lane, **request,
    )
...
def _call_fallback_candidate_sync(
    ...
    destination, fb_kwargs, rebuild = _plan_fallback_candidate(
        ...
        reasoning_config=reasoning_config, fallback_route_ref=fallback_route_ref,
    )
    ...
    try:
        ...
        if retry is not None:
            ...
            try:
                ...
                if not _is_auth_error(retry_err):
                    if _continue_after_aux_fallback_error(
                        task=task, label=fb_label, client=retry[0], exc=retry_err, async_mode=True,
                    ):
                        return None
                    raise
        _quarantine_fallback_candidate(
    ...
    for i, entry in enumerate(chain):
        ...
        if fb_client is not None:
            ...
            if too_small:
                ...
                continue
            _SELECTED_FALLBACK_ROUTE_REF.set(_fallback_route_ref("auxiliary", i, entry))
...
def _get_cached_client(
    ...
    with _client_cache_lock:
        if cache_key in _client_cache:
            ...
            cached_fallback_ref = cached_entry[3] if len(cached_entry) > 3 else None
...
def _prepare_aux_request(
    ...
    pinned_compression_route: Optional[Dict[str, Any]] = None,
) -> _PreparedAuxRequest:
    ...
    if not async_mode and not pinned_compression_route:
    ...
    if pinned_compression_route:
        ...
        effective_call_max_tokens, effective_extra_body, reasoning_config = _fallback_request_controls(
            task, _FallbackDestination(request_provider, base_info or resolved_base_url or "",
                                       resolved_api_mode, final_model),
            str(pinned_compression_route.get("label") or ""), max_tokens=max_tokens,
            effective_extra_body=effective_extra_body, reasoning_config=reasoning_config,
            fallback_route_ref=pinned_ref if isinstance(pinned_ref, _FallbackRouteRef) else None,
        )
    else:
        reasoning_config, effective_extra_body = _compression_route_reasoning(
            task=task, resolved_provider=resolved_provider, resolved_model=resolved_model,
            request_provider=request_provider, final_model=final_model,
            base_url=base_info or resolved_base_url or "", main_runtime=main_runtime,
            reasoning_config=reasoning_config, effective_extra_body=effective_extra_body,
            fallback_route_ref=selected_fallback_ref,
        )
    kwargs = _build_call_kwargs(
        request_provider, final_model, messages, temperature=temperature, max_tokens=effective_call_max_tokens,
    ...
    if effective_call_max_tokens is not None and max_tokens is None:
        kwargs.update(auxiliary_max_tokens_param(effective_call_max_tokens, model=final_model))
    if extra_headers:
...
def _next_fallback_after_quarantine(
    ...
    failed_model: Optional[str], failure_scope: Any, excluded_labels: Optional[set[str]] = None,
) -> Tuple[Optional[Any], Optional[str], str]:
    ...
    fb = _try_configured_fallback_chain(
        ...
        failed_base_url=route.base_info, failure_scope=failure_scope, excluded_labels=excluded_labels)
...
def _ladder_provider_fallback(first_err: Exception, route: _LadderRoute):
    ...
    while True:
        ...
        if fb_client is None:
            break
        if not fb_label or fb_label in excluded_labels:
            logger.warning(
                "Auxiliary %s: configured fallback selector repeated %r; stopping to avoid a retry loop",
...
def _plan_aux_call(
    ...
    pinned_compression_route = (
        ...
        if task == "compression" and isinstance(_pinned_compression_route, dict) else None
    )
    ...
    req = _prepare_aux_request(
        ...
        pinned_compression_route=pinned_compression_route,
    )
...
def _ladder_step_call(
    ...
    return "fallback", step.args, dict(candidate_kwargs, fallback_route_ref=_take_selected_fallback_route_ref())
        ...
        try:
            ...
            for attempt in range(1, max_transient_retries + 1):
                ...
                try:
                    ...
                    if not _should_retry_same_provider(task, retry_transient, " (async)"):
                        raise
