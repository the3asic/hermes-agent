...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _call_summary_llm(self, prompt: str, prompt_started_at: float, *, _main_runtime=None) -> str:
        ...
        call_kwargs: Dict[str, Any] = {
            "task": "compression",
            "main_runtime": self._summary_main_runtime() if _main_runtime is None else _main_runtime,
    ...
    def _generate_summary(
        ...
        *, _main_runtime: Optional[Dict[str, Any]] = None,
        ...
        try:
            content = self._call_summary_llm(prompt, prompt_started_at, _main_runtime=_main_runtime)
