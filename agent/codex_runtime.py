...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Hermes accounting. inputTokens includes cache reads and writes;
    those subsets are removed from uncached input. A turn with no usage still counts as one API call.
