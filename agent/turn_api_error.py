...
def settle_unrecovered_error(
    ...
    if retry_count >= max_retries:
        ...
        if not _retry.primary_recovery_attempted and agent._try_recover_primary_transport(
            ...
            primary_policy = (getattr(agent, "_primary_runtime", None) or {}).get("reasoning_policy_entry")
            agent._runtime_reasoning_entry = dict(primary_policy) if isinstance(primary_policy, dict) else None
            return _verdict("continue")
        if agent._has_pending_fallback():
