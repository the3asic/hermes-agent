...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Dict[str, Any]:
    ...
    for _fb in _fallback_entries(fallback_model):
        ...
        if _fb_client is not None:
            ...
            agent._reasoning_echo_flag = bool(_fb.get("reasoning_echo", False))
            try:
                from agent.chat_completion_helpers import _resolve_reasoning_config_for_active_model
                agent.reasoning_config = _resolve_reasoning_config_for_active_model(agent, agent.model, _fb)
            except Exception as exc:
                agent.reasoning_config = None
                logger.debug("Init-time fallback reasoning resolution failed for %s: %s", agent.model, exc)
