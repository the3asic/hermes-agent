...
2. Explicit selections fail closed when unregistered or incapable; they are never silently replaced.
3. With no selection, the single available capable provider, then ``_LEGACY_PREFERENCE``,
   then the enabled keyless free-tier walk.
4. ``None`` - the tool surfaces the selection or setup error.
"""
...
def _resolve(configured: Optional[str], *, capability: str) -> Optional[WebSearchProvider]:
    ...
    Explicit config wins even when unavailable (so dispatch reports its credential error).
    Invalid or incapable explicit selections return None. With no selection, try the single
    available capable provider, the legacy preference walk, then the keyless free-tier walk.
    """
    ...
    # Explicit selections never silently fall through to another provider.
    if configured:
