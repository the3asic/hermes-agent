...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        resolved_fallback_entry = runtime_kwargs.pop("_resolved_fallback_entry", None)
        ...
        if (confirmed_runtime_lock or session_override or _clean_request_string(session_model)
                or route is not None or request_model or request_provider):
            # An explicit selection is primary, not the global auth-fallback route.
            resolved_fallback_entry = None
        ...
        if isinstance(resolved_fallback_entry, dict):
            request_reasoning_config = _resolve_gateway_reasoning_for_route(
                self, source=None, session_key=gateway_session_key or session_id,
                model=model, fallback_entry=resolved_fallback_entry,
            )
