...
class TurnRunner:
    ...
    def _wire_turn_agent_callbacks(self, agent, turn_route, reasoning_config,
        ...
        from gateway.run_reasoning import (
            _apply_route_reasoning_policy_to_agent, _install_gateway_turn_reasoning_resolver,
        )
        ...
        _install_gateway_turn_reasoning_resolver(runner, agent, source=ctx.source, session_key=ctx.session_key)
    ...
    def run_sync(self):
        ...
        from gateway.run_reasoning import _resolve_gateway_reasoning_for_route
        reasoning_config = _resolve_gateway_reasoning_for_route(
            runner, source=ctx.source, session_key=ctx.session_key, model=model,
        ...
        # Cached agents accumulate usage; only this loop's deltas belong to the turn.
        usage_baseline = {
            arg: getattr(agent, counter, 0) or 0 for arg, counter in (
                ("uncached_input_tokens_start", "session_input_tokens"),
                ("completion_tokens_start", "session_completion_tokens"),
                ("cache_read_tokens_start", "session_cache_read_tokens"),
                ("cache_write_tokens_start", "session_cache_write_tokens"),
                ("usage_report_calls_start", "session_usage_report_calls"),
                ("cache_usage_report_calls_start", "session_cache_usage_report_calls"),
                ("context_usage_report_calls_start", "session_context_usage_report_calls"),
            )
        }
        ...
        usage = _gateway_turn_runtime_metadata(
            agent, **usage_baseline, result_api_calls=result.get("api_calls"),
        )
