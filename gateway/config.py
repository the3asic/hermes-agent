...
class GatewayConfigContractError(ValueError):
    """A typed config contract violation that must not fail open."""
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ChannelOverride":
        ...
        if reasoning_effort is not None:
            ...
            if parsed_reasoning is None:
                raise ValueError(
                    f"reasoning_effort has unknown value {reasoning_effort!r}"
                )
        ...
        if "fallback_providers" in data:
            ...
            for index, entry in enumerate(raw_fallbacks):
                ...
                unknown_fields = sorted(
                    ...
                    if field_name not in FALLBACK_PROVIDER_SUPPORTED_FIELDS
                )
                if unknown_fields:
                    raise ValueError(
                        f"fallback_providers[{index}] has unknown field(s): "
                        + ", ".join(unknown_fields)
                        + ". Supported fields: "
                        + ", ".join(FALLBACK_PROVIDER_SUPPORTED_FIELDS)
                    )
                for optional in FALLBACK_PROVIDER_SUPPORTED_FIELDS:
                    if optional in {"provider", "model"}:
                        continue
                    if (
                        optional in entry
                        and entry[optional] is not None
                        and not isinstance(entry[optional], str)
                    ):
                        raise TypeError(
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        for platform_name, platform_data in platforms_data.items():
            ...
            try:
                ...
                if "channel_overrides" in platform_data:
                    raise ValueError(
                        f"platforms.{platform_name}.channel_overrides belongs to an unknown platform"
                    )
                continue  # Preserve compatibility for unrelated unknown blocks.
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # Legacy top-level platform blocks are not part of ``platforms_data``.
            # Reject a typo that declares channel_overrides here instead of
            # silently skipping the entire policy. Plugin discovery above makes
            # Platform(name) aware of bundled and runtime-registered adapters.
            for _candidate_name, _candidate_cfg in yaml_cfg.items():
                if not (
                    isinstance(_candidate_cfg, dict)
                    and "channel_overrides" in _candidate_cfg
                ):
                    continue
                try:
                    Platform(str(_candidate_name))
                except ValueError as exc:
                    raise GatewayConfigContractError(
                        f"{_candidate_name}.channel_overrides belongs to an unknown platform"
                    ) from exc
    ...
    except GatewayConfigContractError:
        raise
    except Exception as e:
        logger.warning(
