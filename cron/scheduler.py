...
def _resolve_job_runtime(job: dict, job_id: str, jc: _CronJobConfig) -> tuple[dict, str, dict | None]:
    """Resolve the runtime, walking the fallback chain on auth/transient-network errors. Returns
    ``(runtime, model, fallback_entry)``; provider+model swap atomically (never swap only the provider while keeping
    ...
    try:
        ...
        return resolve_runtime_provider(**runtime_kwargs), model, None
        ...
        for entry in get_fallback_chain(jc.cfg):
            ...
            try:
                ...
                return runtime, fb_model, dict(entry)
...
def _resolve_cron_agent_setup(job: dict, job_id: str, job_name: str, jc) -> _CronAgentSetup:
    ...
    setup.runtime, setup.model, setup.fallback_entry = _resolve_job_runtime(job, job_id, jc)
    setup.reasoning_config = _resolve_job_reasoning_config(
        job, _cfg if isinstance(_cfg, dict) else {}, str(setup.model),
        fallback_entry=setup.fallback_entry,
    )
...
def _construct_cron_agent(AIAgent, job: dict, _cfg: dict, setup: _CronAgentSetup, *, workdir, session_id, session_db):
    ...
    apply_initial_reasoning_policy_provenance(agent, setup.fallback_entry, setup.reasoning_config)
    return agent
